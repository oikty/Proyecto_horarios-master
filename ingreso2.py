    ...
    def crear(self,tk,tipoCuenta,nombreCuenta,menuFunc,alumnosFunc,valores=[False]):
        ...
        def cargar():
            ...
            if Cdni != DNIdefault and Cdni.isdigit() is True and len(Cdni) < 8:
                ...
                if valores[0]==True:
                    self.conectar_mysql()
                    self.cursor.execute("UPDATE alumnos SET NOMBRE=%s,APELLIDO=%s,GRUPO=%s,NACIMIENTO=%s,PREFIJO=%s,CODIGO=%s,TELEFONO=%s,nro_de_documento=%s,CURSO=%s WHERE ID=%s",(Cnombre, Capellido, Cgrupo, Cfecha, Cprefijo, Ccodigo, Ctelefono, Cdni, SQLcurso, valores[9]))
                    ...
                    self.conectar_mysql()
                    self.cursor.execute("INSERT INTO alumnos (NOMBRE,APELLIDO,GRUPO,NACIMIENTO,PREFIJO,CODIGO,TELEFONO,nro_de_documento,CURSO) VALUES (%s,%s,%s,%s,%s,%s,%s)",(Cnombre, Capellido, Cgrupo, Cfecha, Cprefijo, Ccodigo, Ctelefono, Cdni, SQLcurso))
                    self.sql.commit()
